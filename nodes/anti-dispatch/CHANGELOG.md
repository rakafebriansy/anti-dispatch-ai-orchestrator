...
- **Perubahan:** `[Fixed]` Memperbaiki pengenalan tombol keputusan pada `extract_state.js` dan fallback `CDPService.swift` dengan normalisasi tokenizer tanda baca/koma/nomor/simbol dan perluasan kata kunci otorisasi (`"allow"`, `"always allow"`, `"allow once"`, `"allow domain"`, `"trust"`, `"accept"`, `"proceed"`), mengatasi kegagalan deteksi tombol berkoma seperti `"Yes, allow this time"` pada prompt izin proxy/domain dinamis (TICKET-09). `[Fixed]` Menambahkan dukungan deteksi opsi radio/checkbox semantik ARIA (`[role="radio"]`, `[role="checkbox"]`, `[data-state="checked"]`) dan pemindaian kartu inline izin di seluruh `agentPanel` yang memuat pertanyaan konfirmasi (TICKET-09). `[Fixed]` Mengeliminasi false positive status WAITING saat agen aktif mengeksekusi Goal/Subagent dengan memvalidasi tombol keputusan riil pada `activeModals`, mengecualikan elemen chat input/monaco toolbar dari `isDecisionButton`, serta memperbaiki hierarki state precedence agar status `RUNNING` dipertahankan saat `hasStopButton` atau `hasSpinner` aktif kecuali ada modal persetujuan eksplisit (TICKET-10). `[Fixed]` Menyaring string instruksi non-keputusan seperti `"Goal"`, `"Task"`, `"Thinking"`, `"Working"` dari parser `promptEl` dan `lines.find` agar tidak keliru dijadikan isi `decisionPrompt` (TICKET-10). `[Added]` Menambahkan unit test `testProxyPermissionPromptDetectedAsWaiting` dan `testActiveGoalExecutionClassifiedAsRunning` pada `Anti_DispatchTests.swift`.
- **Path File:** `Anti Dispatch/Anti Dispatch/Resources/Scripts/extract_state.js`, `Anti Dispatch/Anti Dispatch/Core/Services/CDPService.swift`, `Anti Dispatch/Anti DispatchTests/Anti_DispatchTests.swift`, `anti-dispatch-ai-orchestrator/nodes/anti-dispatch/tickets/TICKET-09-permission-prompt-detection-and-waiting-state.md`, `anti-dispatch-ai-orchestrator/nodes/anti-dispatch/tickets/TICKET-10-working-goal-state-precedence-and-modal-filtering.md`, `anti-dispatch-ai-orchestrator/nodes/anti-dispatch/CHANGELOG.md`

### [2026-09-18 14:30:00] - Guideline: Root Cause Analysis - State Waiting/Working Misclassification in Antigravity (QA-20260918-01)
> **Trigger:** Prompt Driven | **Branch:** `main` | **Repo:** `https://github.com/rakafebriansy/anti-dispatch-ai-orchestrator.git`
- **Konteks:** "kenapa masih ada waiting dianggap done contohnya saat 'Allow testing proxy endpoint on fashionforth.id?' di /Users/raka/Developer/repositories/projects/fashionforth-org/ff-prod. caritahu penyebabnya" dan "dan juga saat Working... di chat namun workingnya saat sedang: Goal Navigate to https://fashionforth.id... dianggap waiting" (Referensi: QA-20260918-01)
- **Perubahan:** `[Added]` Menyusun investigasi komprehensif akar masalah (*root cause analysis*) ke dalam `global-docs/LEARN.md` (entri `[QA-20260918-01]`) terkait anomali deteksi status ganda pada Antigravity: (1) **Waiting -> Done Misclassification**: kegagalan mendeteksi tombol berkoma (`"Yes, allow this time"`), domain dinamis, dan class Tailwind inline pada izin proxy sehingga status jatuh ke `DONE`, serta (2) **Working -> Waiting False Positive**: dialog/panel Goal subagent dengan `role="dialog"` memicu `activeModals` tanpa validasi tombol izin, keyword tombol generik (`submit`/`skip`/`no`), dan pembajakan status `RUNNING` oleh `isWaitingForInput` yang mengekstrak teks instruksi Goal menjadi `decisionPrompt`.
- **Path File:** `anti-dispatch-ai-orchestrator/global-docs/LEARN.md`, `anti-dispatch-ai-orchestrator/nodes/anti-dispatch/CHANGELOG.md`

### [2026-09-18 08:15:00] - Implementation: Swift 6 Concurrency Deinit Isolation & macOS 14+ Deprecation Fixes
> **Trigger:** Prompt Driven | **Branch:** `main` | **Repo:** `https://github.com/rakafebriansy/anti-dispatch-ai-orchestrator.git`
- **Konteks:** Memperbaiki peringatan deprecation `'activateIgnoringOtherApps' was deprecated in macOS 14.0`, peringatan compiler `Result of call to 'run(resultType:body:)' is unused`, serta galat Swift 6 Concurrency `Call to main actor-isolated instance method 'orderOut' in a synchronous nonisolated context`.
- **Perubahan:** `[Fixed]` Memperbarui pemanggilan aktivasi aplikasi pada `AppDelegate.swift`, `NotchViewModel.swift`, dan `BranchCreatorSheet.swift` dengan guard `#available(macOS 14.0, *)` untuk memanggil modern `.activate()` tanpa parameter usang pada macOS 14+. `[Fixed]` Menghilangkan peringatan compiler `Result of call to 'run(resultType:body:)' is unused` pada `BranchCreatorSheet.swift` dan `NotchViewModel.swift` dengan menerapkan isolasi `Task { @MainActor in ... }` langsung daripada memanggil nested `MainActor.run`. `[Fixed]` Mengatasi galat isolasi aktor pada deinitialisasi `NotchPanelController.swift` dan `StatusBarController.swift` dengan mendeklarasikan `isolated deinit` agar pemanggilan method UI AppKit (`orderOut`, `close`, `removeStatusItem`) tetap terisolasi secara aman di Main Thread.
- **Path File:** `Anti Dispatch/Anti Dispatch/App/AppDelegate.swift`, `Anti Dispatch/Anti Dispatch/Presentation/Notch/ViewModels/NotchViewModel.swift`, `Anti Dispatch/Anti Dispatch/Presentation/Modals/BranchCreatorSheet.swift`, `Anti Dispatch/Anti Dispatch/Presentation/Notch/NotchPanelController.swift`, `Anti Dispatch/Anti Dispatch/Presentation/MenuBar/StatusBarController.swift`, `anti-dispatch-ai-orchestrator/nodes/anti-dispatch/CHANGELOG.md`

### [2026-09-17 16:50:00] - Implementation: Decision Required HUD Bug Fixes, Realtime Detection & Status Bar UX Overhaul (TICKET-08)
> **Trigger:** Prompt Driven | **Branch:** `main` | **Repo:** `https://github.com/rakafebriansy/anti-dispatch-ai-orchestrator.git`
- **Konteks:** "1. pada Decision Required modal masih terlihat container dengan opacity kecil dibelakangnya... 2. hover animation jump to app jangan dibuat blinking... 3. saat klik button jump to app di Decision Required kadang tidak berdampak apa apa... 4. di status bar, icon anti dispatch pakai icon custom? ... 5. menu status bar dibuat lebih interaktif dan ui ux friendly... 6. terdapat bug dimana sekarang decision required modal tidak realtime akibat penyelesaian critical 02 dan high 06" (Referensi: TICKET-08)
- **Perubahan:** `[Fixed]` Menghilangkan artifact ghost box/container opacity di belakang Decision Required HUD dengan mengonfigurasi `NSHostingView` layer (`wantsLayer = true`, `backgroundColor = .clear`) pada `DecisionNotificationHUDController.swift`. `[Fixed]` Memperbaiki animasi hover tombol "Jump to App" agar tidak blinking/flickering dengan memindahkan state transition ke view-level `.animation(.easeInOut(duration: 0.18))` serta menambahkan `.contentShape(Rectangle())` dan dynamic glow shadow. `[Fixed]` Memperbaiki tombol "Jump to App" yang tidak responsif dengan memindahkan eksekusi AppleScript dan AXUIElement ke `Task.detached(priority: .userInitiated)` serta menerapkan `.activate(options: [.activateIgnoringOtherApps, .activateAllWindows])` secara instan tanpa memblokir Main Thread. `[Fixed]` Mengatasi regresi deteksi realtime Decision Required HUD dari TICKET-07 dengan mempercepat interval polling saat ada sesi `.waiting` (`hasActive`), serta memicu pembaruan `decisionHUD?.update()` secara instan saat sesi berpindah ke state `.waiting` atau ditambahkan/dihapus. `[Added]` Menambahkan dukungan custom status bar template icon (`StatusBarIcon`) dari asset bundle pada `StatusBarController.swift` dengan fallback dinamis. `[Changed]` Merombak total antarmuka menu status bar macOS dengan header ringkasan multi-status, daftar active workspaces berindikator status berwarna (`●`, `⚠️`, `✓`, `○`), submenu detail & navigasi per workspace, quick actions berikon SF Symbols, dan visual toggle status notch.
- **Path File:** `Anti Dispatch/Anti Dispatch/Presentation/Notifications/DecisionNotificationHUDController.swift`, `Anti Dispatch/Anti Dispatch/Presentation/Notch/ViewModels/NotchViewModel.swift`, `Anti Dispatch/Anti Dispatch/Presentation/MenuBar/StatusBarController.swift`, `anti-dispatch-ai-orchestrator/nodes/anti-dispatch/tickets/TICKET-08-decision-hud-and-statusbar-ux.md`, `anti-dispatch-ai-orchestrator/nodes/anti-dispatch/CHANGELOG.md`

### [2026-09-17 16:40:00] - Implementation: Immediate Startup Scan & Interactive Instant Wakeup Probing (TICKET-07 Refinement)
> **Trigger:** Prompt Driven | **Branch:** `main` | **Repo:** `https://github.com/rakafebriansy/anti-dispatch-ai-orchestrator.git`
- **Konteks:** "berarti kekurangan 1.3 saat startup bisa lambat jika tidak beruntung? ... 1 dan 3 saja" (Referensi: TICKET-07)
- **Perubahan:** `[Added]` Menambahkan metode `triggerImmediateScan()` pada `NotchViewModel` untuk memicu pemindaian port penuh seketika saat terjadi interaksi pengguna. `[Changed]` Memodifikasi logika probing pada `NotchViewModel.swift` agar siklus pertama (`isFirstTick = probeTickCount == 1`) selalu melakukan full port scan tanpa jeda, mengeliminasi potensi keterlambatan deteksi Antigravity saat startup. `[Changed]` Mengintegrasikan pemanggilan `viewModel.triggerImmediateScan()` pada `DynamicNotchRootView.swift` saat notch disentuh kursor (`handleHover`), diklik (`onTapGesture`), atau dibuka (`onChange(of: shouldBeExpanded)`), menjamin deteksi instan instance Antigravity baru yang dibuka dari luar (Terminal/Finder).
- **Path File:** `Anti Dispatch/Presentation/Notch/ViewModels/NotchViewModel.swift`, `Anti Dispatch/Presentation/Notch/Views/DynamicNotchRootView.swift`, `nodes/anti-dispatch/tickets/TICKET-07-memory-and-resource-optimization.md`, `nodes/anti-dispatch/CHANGELOG.md`

### [2026-09-17 16:12:00] - Implementation: Memory & Background Resource Optimization (TICKET-07)
> **Trigger:** Prompt Driven | **Branch:** `main` | **Repo:** `https://github.com/rakafebriansy/anti-dispatch-ai-orchestrator.git`
- **Konteks:** "periksalah apakah sudah sangat hemat memori aplikasinya? periksalah penggunaan memori, potensi memori leak, efisiensi background process, dll. buatlah markdown report terkait hal tersebut dan apakah bisa diperbaiki/ditingkatkan! ... buatlah implementation plannya" (Referensi: TICKET-07)
- **Perubahan:** `[Fixed]` Mengeliminasi penumpukan *unbounded concurrent background tasks* pada pencarian git branch dengan tracking dan auto-cancellation via `pendingBranchTasks` di `NotchViewModel`. `[Fixed]` Menghilangkan *expensive process spawning* (`lsof`/`ps`) berulang setiap 1 detik dengan menambahkan `resolvedWorkspaceCache` berbasis TTL 10 detik. `[Changed]` Mengonversi `URLSession.shared` pada `CDPService` menjadi ephemeral `URLSession(configuration: .ephemeral)` untuk menonaktifkan cache memori/disk yang tidak perlu. `[Changed]` Mengoptimalkan duplikasi string fallback JavaScript extractor pada instance `CDPService` menjadi `private static let fallbackScript`. `[Changed]` Mengurangi beban probing socket TCP range non-terkoneksi menjadi setiap 3 siklus (*throttled*). `[Changed]` Men-gate render loop animasi equalizer audio bar agar hanya berjalan saat notch dibuka (`isContainerExpanded`) dan melonggarkan interval timer animasi & hover check. `[Changed]` Membatasi ukuran buffer `latestResponse` ke 500 karakter untuk mencegah lonjakan memori akibat payload log/output besar. `[Added]` Menambahkan unit test `testLatestResponseTruncation` dan `testCDPServiceCustomScriptInit` pada `Anti_DispatchTests.swift`.
- **Path File:** `Anti Dispatch/Core/Services/CDPService.swift`, `Anti Dispatch/Presentation/Notch/ViewModels/NotchViewModel.swift`, `Anti Dispatch/Presentation/Notch/Views/DynamicNotchRootView.swift`, `Anti DispatchTests/Anti_DispatchTests.swift`, `nodes/anti-dispatch/tickets/TICKET-07-memory-and-resource-optimization.md`, `nodes/anti-dispatch/CHANGELOG.md`
...
- **Perubahan:** `[Fixed]` Memperbaiki masalah teks label pada modal keputusan yang jatuh ke string fallback umum dengan mempersempit scope ekstraksi DOM langsung ke kontainer modal izin yang aktif (`[role="radiogroup"]` -> `parentElement` / `[tabindex="-1"]`) alih-alih seluruh kontainer `agentPanel`, mengambil elemen `span` pertanyaan terakhir yang relevan (`promptEls[promptEls.length - 1]`), serta memperbaiki hierarki traversal fallback `radioOrCheckboxes` agar menargetkan kontainer form/modal secara utuh. `[Fixed]` Menyelaraskan seluruh logika JS pada inline fallback `CDPService.swift`. `[Added]` Menambahkan unit test `testDecisionPromptDecodingInWaitingState` pada `Anti_DispatchTests.swift`.
- **Path File:** `Anti Dispatch/Resources/Scripts/extract_state.js`, `Anti Dispatch/Core/Services/CDPService.swift`, `Anti DispatchTests/Anti_DispatchTests.swift`, `nodes/anti-dispatch/tickets/TICKET-05-decision-prompt-scoped-modal-extraction.md`, `nodes/anti-dispatch/CHANGELOG.md`


### [2026-09-16 18:05:00] - Implementation: Stuck RUNNING State Fix & Prompt Selector Simplification (TICKET-04)
> **Trigger:** Prompt Driven | **Branch:** `main` | **Repo:** `https://github.com/rakafebriansy/anti-dispatch-ai-orchestrator.git`
- **Konteks:** "kenapa prompt selectornya banyak sekali? tidakkah kamu buat selector spesifik yang pasti? ... saat ini, status instance adalah working, padahal sudah selesai: Saya telah menyusun rencana implementasi untuk TICKET-30: Pembuatan API Route Katalog Pasar Prediksi pada artefak implementation_plan.md ... carilah penyebabnya" (Referensi: TICKET-04)
- **Perubahan:** `[Fixed]` Memperbaiki masalah status agen yang terkunci di `RUNNING`/`Working` dengan mempersempit scope query elemen aktif (`hasSpinner`, `hasActiveTaskElement`, `hasStatusBarRunning`) khusus di dalam `agentPanel`, menyingkirkan selector UI internal Monaco editor (`.monaco-progressbar`, `[role="progressbar"]`, `[class*="loading"]`), memindahkan ekstraksi `latestResponse` sebelum evaluasi state, menambahkan guard `hasDefinitelyFinished = latestResponse && !hasStopButton`, serta mengabaikan label accordion step lampau jika respons asisten sudah lengkap (`!latestResponse && steps.some(...)`). `[Changed]` Menyederhanakan 14 selector fallback decision prompt menjadi satu query spesifik tunggal `span.font-medium.text-foreground` di dalam `agentPanel`. `[Fixed]` Menyelaraskan seluruh perubahan logika di atas ke dalam skrip inline fallback `CDPService.swift`. `[Added]` Menambahkan unit test `testStateResetsToDoneWhenResponseCompleteAndNoStopButton` pada `Anti_DispatchTests.swift`.
- **Path File:** `Anti Dispatch/Resources/Scripts/extract_state.js`, `Anti Dispatch/Core/Services/CDPService.swift`, `Anti DispatchTests/Anti_DispatchTests.swift`, `nodes/anti-dispatch/tickets/TICKET-04-state-stuck-fix-and-prompt-selector.md`, `nodes/anti-dispatch/CHANGELOG.md`

### [2026-09-16 17:58:00] - Implementation: Decision Required Modal Action Prompt Label Extraction (TICKET-03)
> **Trigger:** Prompt Driven | **Branch:** `main` | **Repo:** `https://github.com/rakafebriansy/anti-dispatch-ai-orchestrator.git`
- **Konteks:** "di Decision Required modal kenapa muncul penjelasan yang irelevan seperti contohnya muncul "POST" padahal teks aslinya "Allow committing and pushing TICKET-28 in orchestrator?" (label promptnya, bukan di dalam textbox dan bukan optionnya) ... saat ini yang muncul di decision required modal adalah "Run git pull --rebase origin main && git add..." padahal yang saya mau adalah labelnya, seperti: Allow syncing and committing TICKET-29 in orchestrator?" (Referensi: TICKET-03)
- **Perubahan:** `[Fixed]` Memperbaiki ekstraksi label prompt keputusan pada `extract_state.js` dan fallback `CDPService.swift` dengan memindai elemen label pertanyaan aksi secara spesifik (`span.text-foreground`, `span.font-medium`, selector judul dialog, dan penelusuran hierarki kartu kontainer dari tombol interaktif) yang memuat kalimat pertanyaan (berakhiran `?` atau diawali frasa aksi `Allow `, `Confirm `, `Do you `, `Would you `), sehingga label prompt pertanyaan manusia seperti *"Allow syncing and committing TICKET-29 in orchestrator?"* selalu diprioritaskan penuh dan tidak tertimpa oleh blok kode perintah terminal di dalamnya (`Run git pull...`). `[Added]` Menambahkan unit test `testActionApprovalPromptTextPrecedenceOverTerminalCommand` pada `Anti_DispatchTests.swift`.
- **Path File:** `Anti Dispatch/Resources/Scripts/extract_state.js`, `Anti Dispatch/Core/Services/CDPService.swift`, `Anti DispatchTests/Anti_DispatchTests.swift`, `nodes/anti-dispatch/tickets/TICKET-03-background-state-and-workspace-naming.md`, `nodes/anti-dispatch/CHANGELOG.md`

### [2026-09-16 17:41:00] - Implementation: Background Process State Persistence & Untitled Workspace Resolution (TICKET-03)
> **Trigger:** Prompt Driven | **Branch:** `main` | **Repo:** `https://github.com/rakafebriansy/anti-dispatch-ai-orchestrator.git`
- **Konteks:** "1. saat background process sesuatu misal npm install, testing, dll. masih didefine sebagai done, harusnya tetap processing 2. saat muncul modal decision required di directory antigravity yang berlabel "Untitled (Workspace)" (karena saya menambahkan folder di workspace dan tidak memberi nama workspace) gantilah ke nama folder aslinya sebelum menambahkan folder lain ke workspace" (Referensi: TICKET-03)
- **Perubahan:** `[Fixed]` Memperbaiki logika deteksi status agen pada `extract_state.js` dan fallback `CDPService.swift` dengan memperluas selector deteksi elemen proses latar belakang aktif (`[data-status="running"]`, `.codicon-modifier-spin`, `[aria-busy="true"]`, badge task chip) dan pemindaian kata kerja aksi in-progress (`running`, `installing`, `testing`, `building`, `executing`) pada subSteps yang belum selesai, sehingga status tetap dipertahankan pada state Processing / Working (`RUNNING`) dan tidak prematur berganti ke `DONE` saat respons teks asisten muncul. `[Fixed]` Memperbaiki resolusi penamaan workspace pada `extract_state.js` dan `NotchViewModel.swift` saat jendela Antigravity berlabel `"Untitled (Workspace)"` dengan mengekstrak nama folder asli secara hierarkis dari parameter metadata environment `vscode-workbench-environment-params`, konfigurasi web `vscode-workbench-web-configuration`, DOM Explorer header, serta resolusi URL path proses (`lastPathComponent`), memastikan nama folder sebenarnya selalu ditampilkan pada antarmuka notch dan modal `DecisionNotificationHUDView`. `[Changed]` Mengubah properti `workspaceName` menjadi mutable pada `WorkspaceSession.swift`, menambahkan helper computed property `displayName` & auto-sanitization pada `WorkspaceSession.init`, serta memperbarui seluruh tampilan UI (`MultiSessionAccordionView`, `SingleSessionView`, `StatusBarController`, dan `DecisionNotificationHUDView`) agar menggunakan `session.displayName`. `[Added]` Menambahkan unit test `testRunningStateWithBackgroundProcess`, `testDecisionHUDDisplaysRealFolderName`, dan `testWorkspaceSessionDisplayNameResolution` pada `Anti_DispatchTests.swift`.
- **Path File:** `Anti Dispatch/Resources/Scripts/extract_state.js`, `Anti Dispatch/Core/Services/CDPService.swift`, `Anti Dispatch/Core/Models/WorkspaceSession.swift`, `Anti Dispatch/Presentation/Notch/ViewModels/NotchViewModel.swift`, `Anti Dispatch/Presentation/Notch/Views/MultiSessionAccordionView.swift`, `Anti Dispatch/Presentation/Notch/Views/SingleSessionView.swift`, `Anti Dispatch/Presentation/MenuBar/StatusBarController.swift`, `Anti Dispatch/Presentation/Notifications/DecisionNotificationHUDController.swift`, `Anti DispatchTests/Anti_DispatchTests.swift`, `nodes/anti-dispatch/tickets/TICKET-03-background-state-and-workspace-naming.md`, `nodes/anti-dispatch/CHANGELOG.md`

### [2026-09-16 14:36:00] - Implementation: CDP WebSocket Stabilization & Polling Grace Period
> **Trigger:** Prompt Driven | **Branch:** `main` | **Repo:** `https://github.com/rakafebriansy/anti-dispatch-ai-orchestrator.git`
- **Konteks:** "state sering kali hilang-hilang, apakah karena websocket?"
- **Perubahan:** `[Fixed]` Memperbaiki masalah state hilang-muncul (*flickering*) dengan mengisolasi `pendingRequests` per port (`[Int: [Int: CheckedContinuation]]`) pada `CDPService` agar diskoneksi satu port tidak membatalkan request port lain. `[Changed]` Meningkatkan timeout HTTP probe menjadi `1.5s` dan timeout evaluasi WebSocket menjadi `1.5s` agar tahan terhadap beban kerja CPU saat Antigravity sedang sibuk. `[Added]` Menambahkan mekanisme *Grace Period* (`missedProbeCount`) pada `NotchViewModel` yang mempertahankan state sesi terakhir dan hanya menghapus sesi dari UI setelah 4 kali kegagalan probe berturut-turut ($\ge 4$ detik) untuk mencegah penghapusan prematur akibat penurunan frame sesaat.
- **Path File:** `Anti Dispatch/Core/Services/CDPService.swift`, `Anti Dispatch/Presentation/Notch/ViewModels/NotchViewModel.swift`, `nodes/anti-dispatch/CHANGELOG.md`

### [2026-09-16 14:28:00] - Implementation: Decision HUD Queue System & Multi-Tier Git Directory Detection
> **Trigger:** Prompt Driven | **Branch:** `main` | **Repo:** `https://github.com/rakafebriansy/anti-dispatch-ai-orchestrator.git`
- **Konteks:** "1. pada popup/modal Decision required, jika ada lebih dari 1 decision tambahkan di dalam modal informasi "queue: 3", yang mana jika modal sudah diklik jump atau ignore, maka modal menutup dan memunculkan modal baru sesuai urutan queue yang butuh decision. 2. terdapat kecacatan dalam mendeteksi git directory, pada /Users/raka/Developer/repositories/projects/wealthy-people-org/omen-dir/omen dicatat sebagai folder biasa dan tidak bisa dibuat worktree. padahal memiliki git"
- **Perubahan:** `[Added]` Menambahkan indikator antrean `"queue: N"` (misal `"queue: 3"`) pada header modal `DecisionNotificationHUDView` ketika ada lebih dari 1 sesi yang menunggu keputusan user, serta mengatur alur tombol `Jump to App` dan `Ignore` pada `DecisionNotificationHUDController` agar langsung memproses aksi sesi saat ini dan segera memunculkan pop-up sesi berikutnya dari antrean hingga seluruh decision selesai. `[Fixed]` Memperbaiki resolusi Git repository pada `GitWorktreeService` dengan implementasi multi-tier `findGitRoot` yang mendeteksi `.git` baik pada target repo langsung, subfolder di dalamnya (`omen/web`), maupun parent/wrapper folder (`omen-dir` berisi `omen/`), memperbarui sanitasi URL/path, menangani output subprocess Git secara utuh, serta mengekstrak `workspacePath` dari DOM Electron/CDP dan process tree pada `NotchViewModel`. `[Added]` Menambahkan unit test `testGitAndRegularFolderDetection` (multi-tier parent/nested check) dan `testDecisionHUDQueueInitialization` pada `Anti_DispatchTests.swift`.
- **Path File:** `Anti Dispatch/Presentation/Notifications/DecisionNotificationHUDController.swift`, `Anti Dispatch/Core/Services/GitWorktreeService.swift`, `Anti Dispatch/Core/Services/CDPService.swift`, `Anti Dispatch/Resources/Scripts/extract_state.js`, `Anti Dispatch/Presentation/Notch/ViewModels/NotchViewModel.swift`, `Anti Dispatch/Presentation/Modals/BranchCreatorSheet.swift`, `Anti DispatchTests/Anti_DispatchTests.swift`, `nodes/anti-dispatch/CHANGELOG.md`
